...
use ignitify_db::{Database, ProviderAuthMode, ProviderKind, ProviderRecord};
use ignitify_domain::{
...
impl GitSourceBuild {
        ...
        let clone_result = self
            .run(
                Command::new(&self.git_bin)
                    ...
                    .args(["clone", "--depth", "1", "--no-tags", "--branch", branch])
                    .arg(&remote)
                    .arg(&path),
                "git checkout",
            )
    ...
    async fn output(
        ...
        let output = command.stderr(Stdio::null()).output().await?;
...
mod tests {
    use super::{
        compose_runtime_spec, is_git_revision, is_local_image_id, relative_path, shell_quote,
        static_dockerfile,
    };
