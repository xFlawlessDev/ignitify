...
5. In **Infrastructure > Ingress & TLS**, set that control-plane hostname, the
   application domain suffixes, one per managed domain family, an ACME contact
   email, and automatic certificates (or select a custom certificate). Create
   a wildcard DNS record for each suffix. Ignitify generates the Traefik HTTPS route
   to its loopback-only backend. The control-plane hostname may be within the
   managed application suffixes, but is reserved from service routes.
